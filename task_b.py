...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        ''' 
        #Agent String indicate the initial position of agent
        
        agent_string = '' 
        agent_x = self.state.agent.position.x
        agent_y = self.state.agent.position.y

        agent_string += "(at " + "agent1 "+"pt{}pt{}t{}".format(agent_x, agent_y, 0)+") "

        #Blocked String shows the positions of parked cars
        blocked_string = ''
        for car in self.state.cars:
            ...
            for t in range(self.width):
                if car_x >=0:
                    #blocked_list.append("pt{}pt{}t{}".format(car_x, car_y, t))
                    blocked_string += "(blocked " +"pt{}pt{}t{}".format(car_x, car_y, t)+") "
                    car_x -= 1
        ...
        for w in range(self.width-1, 0, -1):
            for lane in range(self.num_lanes):
                for t in range(self.width-1):
                    #forward_next_list.append(("pt{}pt{}t{}".format(w, lane, t),"pt{}pt{}t{}".format(w-1, lane, t+1)))
                    predicate_string += "(forward_next " +"pt{}pt{}t{}".format(w, lane, t)+ " " +"pt{}pt{}t{}".format(w-1, lane, t+1)+") "
                    if lane < self.num_lanes-1:
                        #down_next_list.append(("pt{}pt{}t{}".format(w, lane, t),"pt{}pt{}t{}".format(w-1, lane+1, t+1)))
                        predicate_string += "(down_next " +"pt{}pt{}t{}".format(w, lane, t)+ " " +"pt{}pt{}t{}".format(w-1, lane+1, t+1)+") "
                    if lane > 0:
                        #up_next_list.append(("pt{}pt{}t{}".format(w, lane, t),"pt{}pt{}t{}".format(w-1, lane-1, t+1)))
                        predicate_string += "(up_next " +"pt{}pt{}t{}".format(w, lane, t)+ " " +"pt{}pt{}t{}".format(w-1, lane-1, t+1)+") "


        return agent_string + blocked_string + predicate_string
    ...
    def generateGoalString(self) :
        ...
        '''
        finish_string = ''   
        finish_x = self.state.finish_position.x
        finish_y = self.state.finish_position.y
        finish_string += "(at " + "agent1 "+"pt{}pt{}t{}".format(finish_x, finish_y, self.width - 1)+")"
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="UP", 
                  parameters=(("pt1", "gridcell"), ("car" , "car"), ("pt2", "gridcell")), 
                  #precondition_string="(and (at ?car ?pt1) (up_next ?pt1 ?pt2))", 
                  precondition_string="(and (at ?car ?pt1) (up_next ?pt1 ?pt2) (not (blocked ?pt2)))", 
                  effect_string= "(and (not (at ?car ?pt1)) (at ?car ?pt2))")
    gen.addAction(name="DOWN", 
                  parameters=(("pt1", "gridcell"), ("car" , "car"), ("pt2", "gridcell")), 
                  #precondition_string="(and (at ?car ?pt1) (down_next ?pt1 ?pt2))", 
                  precondition_string="(and (at ?car ?pt1) (down_next ?pt1 ?pt2) (not (blocked ?pt2)))", 
                  effect_string= "(and (not (at ?car ?pt1)) (at ?car ?pt2))")
    gen.addAction(name="FORWARD", 
                  parameters=(("pt1", "gridcell"), ("car" , "car"), ("pt2", "gridcell")), 
                  #precondition_string="(and (at ?car ?pt1) (forward_next ?pt1 ?pt2))", 
